...
export default function AdminDashboard() {
    ...
    const [dashboardData, setDashboardData] = React.useState<any>({
        stats: { totalProducts: 0, totalVariations: 0, totalPorts: 0, totalCurrencies: 0, totalAlerts: 0, topGainers: [], planDistribution: {} },
        latestUsers: [],
        latestAnnouncements: [],
        systemHealth: { dbLatency: '0ms', uptime: '100%', securityStatus: 'Protected' },
        aiInsight: { summary: 'Analyzing market data...', sentiment: 'Calculating...' }
    });
        ...
        const fetchDashboardData = async () => {
            try {
                ...
                if (statsRes.ok) {
                    ...
                    setDashboardData((prev: any) => ({ ...prev, ...data }));
                }
                
                if (aiRes.ok) {
                    const aiData = await aiRes.json();
                    setDashboardData((prev: any) => ({ ...prev, aiInsight: aiData }));
                }
            } catch (error) {
    ...
    const stats = [
        { label: 'Total Products', value: dashboardData.stats.totalProducts.toString(), change: '+2.5%', icon: <ProductIcon />, color: '#3b82f6' },
        { label: 'Variations', value: dashboardData.stats.totalVariations.toString(), change: '+1.2%', icon: <VariationIcon />, color: '#8b5cf6' },
        { label: 'Active Alerts', value: dashboardData.stats.totalAlerts.toString(), change: '+12%', icon: <AnnouncementIcon />, color: '#f59e0b' },
        { label: 'Currencies', value: dashboardData.stats.totalCurrencies.toString(), change: 'Stable', icon: <CurrencyIcon />, color: goldColor },
    ];
        ...
        <Box sx={{ flexGrow: 1 }}>
            ...
            <Paper 
                sx={{ 
                    ...
                    p: 2.5, 
                    borderRadius: 4, 
                    background: `linear-gradient(90deg, ${alpha(goldColor, 0.05)} 0%, ${alpha(navyColor, 0.03)} 100%)`,
            ...
            >
                <Avatar sx={{ bgcolor: goldColor, color: '#fff', width: 44, height: 44 }}>
                    <ActionIcon />
                </Avatar>
                <Box sx={{ flexGrow: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
                        <Typography variant="subtitle2" sx={{ fontWeight: 800, color: goldColor, textTransform: 'uppercase', letterSpacing: 1 }}>
                            AI Strategic Insight
                        </Typography>
                        <Chip 
                            label={dashboardData.aiInsight.sentiment} 
                            size="small" 
                            sx={{ bgcolor: goldColor, color: '#fff', fontWeight: 800, height: 18, fontSize: '0.65rem' }} 
                        />
                    </Box>
                    <Typography variant="body2" sx={{ color: navyColor, fontWeight: 600, fontStyle: 'italic' }}>
                        "{dashboardData.aiInsight.summary}"
                    </Typography>
                </Box>
            </Paper>
            ...
            <Grid container spacing={3}>
                ...
                <Grid size={{ xs: 12, lg: 4 }}>
                    <Grid container spacing={3}>
                            ...
                            <Paper sx={{ p: 3, borderRadius: 5, bgcolor: navyColor, color: '#fff', position: 'relative', overflow: 'hidden' }}>
                                ...
                                <Grid container spacing={1.5}>
                                    {[
                                        ...
                                        { label: 'Audit Trail', link: '/admin/audit-logs', icon: <AuditIcon fontSize="small" /> },
                        ...
                        <Grid size={{ xs: 12 }}>
                            <Paper sx={{ p: 3, borderRadius: 5, border: '1px solid rgba(0,0,0,0.05)', position: 'relative' }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                                    <Typography variant="h6" sx={{ fontWeight: 800, color: navyColor }}>Security Sentinel</Typography>
                                    <Chip 
                                        label={dashboardData.systemHealth.securityStatus} 
                                        size="small" 
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <Typography variant="body2" sx={{ color: 'text.secondary', display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <PortIcon sx={{ fontSize: 16 }} /> DB Latency
                                        </Typography>
                                        <Typography variant="body2" sx={{ fontWeight: 800, color: navyColor }}>{dashboardData.systemHealth.dbLatency}</Typography>
                                    </Box>
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <Typography variant="body2" sx={{ color: 'text.secondary', display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <RefreshIcon sx={{ fontSize: 16 }} /> System Uptime
                                        </Typography>
                                        <Typography variant="body2" sx={{ fontWeight: 800, color: '#10b981' }}>{dashboardData.systemHealth.uptime}</Typography>
                                    </Box>
                                </Box>
                                <Divider sx={{ my: 2 }} />
                                <Typography variant="caption" sx={{ color: 'text.secondary', fontStyle: 'italic', display: 'block' }}>
                                    External API Status: <span style={{ color: '#10b981', fontWeight: 700 }}>Healthy</span>
                                </Typography>
                            </Paper>
                        </Grid>

                        <Grid size={{ xs: 12 }}>
                            <Paper sx={{ p: 3, borderRadius: 5, border: '1px solid rgba(0,0,0,0.05)', position: 'relative', overflow: 'hidden' }}>
                                <Box sx={{ position: 'absolute', right: -10, bottom: -10, opacity: 0.03 }}>
                                    <TrendingUp sx={{ fontSize: 100 }} />
                                </Box>
                                <Typography variant="h6" sx={{ fontWeight: 800, mb: 2, color: navyColor }}>Market Volatility</Typography>
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                    {dashboardData.stats.topGainers.map((gainer: any) => (
                                        <Box key={gainer.name} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <Typography variant="body2" sx={{ fontWeight: 700 }}>{gainer.name}</Typography>
                                            <Typography variant="body2" sx={{ color: '#10b981', fontWeight: 800 }}>+{gainer.trend}%</Typography>
                                        </Box>
                                    ))}
                                    {dashboardData.stats.topGainers.length === 0 && <Typography variant="caption">Monitoring market signals...</Typography>}
                                </Box>
                                <Button fullWidth variant="outlined" sx={{ mt: 3, borderRadius: 2, textTransform: 'none', fontWeight: 700, borderColor: alpha(navyColor, 0.1), color: navyColor }}>
                                    Deep Analysis
                                </Button>
                            </Paper>
                        </Grid>
