...
import { FaTachometerAlt, FaUserCircle, FaChartBar, FaBoxOpen, FaEnvelope, FaSignOutAlt, FaBell } from 'react-icons/fa'
...
export default function UserDashboard() {
    ...
    const { permission, subscribeToPush, loading: pushLoading } = usePushNotifications()

    const handleTestNotification = async () => {
        try {
            const res = await fetch('/api/user/test-notification', { method: 'POST' })
            const data = await res.json()
            if (!res.ok) throw new Error(data.error)
            alert(data.message || 'Test notification sent! Check your device center.')
        } catch (error: any) {
            ...
            <main className={styles.mainContent}>
                ...
                <div className={styles.dashboardGrid}>
                    ...
                    <div className={styles.card}>
                        ...
                        <Link href="/products" className={styles.cardButton} style={{ display: 'inline-block', textAlign: 'center' }}>
                            Browse Products
                        </Link>
                    </div>
                    ...
                    <div className={styles.card}>
                        ...
                        <p className={styles.cardDescription} style={{ marginBottom: '1rem' }}>
                            Status: <span style={{ fontWeight: 600, color: permission === 'granted' ? '#10b981' : permission === 'denied' ? '#ef4444' : '#f59e0b' }}>{permission.toUpperCase()}</span>
                        </p>

                        <div style={{ display: 'flex', gap: '10px', flexDirection: 'column' }}>
                            {permission !== 'granted' && (
                                <button onClick={subscribeToPush} disabled={pushLoading} className={styles.cardButton} style={{ width: '100%', background: '#f59e0b', color: 'white', border: 'none' }}>
                                    {pushLoading ? 'Enabling...' : 'Enable Push Alerts'}
                                </button>
                            )}
                            {permission === 'granted' && (
                                <button onClick={handleTestNotification} className={styles.cardButton} style={{ width: '100%', background: '#3b82f6', color: 'white', border: 'none' }}>
